...
// Chat routes (for approved therapists) - handled by the comprehensive route below
...
router.get(
  ...
  async (req, res) => {
    try {
      ...
      await ensureAdminConversation(therapistId);
...
async function ensureAdminConversation(therapistId) {
  ...
  try {
    ...
    if (existingResult.rows.length > 0) {
      return; // Admin conversation already exists
    }
    ...
    const createQuery = `
      ...
      RETURNING id
    `;
    await db.query(createQuery, [therapistId, adminUser.id]);

    console.log(`Created admin conversation for therapist ${therapistId} with admin ${adminUser.id}`);
  } catch (error) {
    ...
    try {
      ...
      const verifyQuery = `
        ...
        WHERE c.id = $1
          ...
          AND (c.status IS NULL OR c.status = 'active')
      `;
      const verifyResult = await db.query(verifyQuery, [conversationId, therapistId]);
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      `;
      const messageResult = await db.query(messageQuery, [conversationId, therapistId, content.trim()]);
      ...
      try {
        ...
        `;
        await db.query(notificationQuery, [adminId, JSON.stringify({
